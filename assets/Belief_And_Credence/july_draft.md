...
This account is inspired by, and can be seen as a version of, the approach to belief and credence sketched in @macfarlane2025BeliefWhat; but it goes a bit beyond what MacFarlane does in his paper, though it certainly considers itself to be a *MacFarlanian* account of belief and credence.

The key premises my approach is built upon are as follows:

1. A methodological behaviourism about belief and credence. That is, I will approach these concepts as theoretical terms, part of the vocabulary of our psychological theories which need to justify themselves in light of their power to help us make sense of, explain and predict the observable behaviour of human beings.
2. I assume there is a specific normativity which applies to actions properly governed by decision theory (which, I will assume, are those actions for which both doing and refraining from doing them is permitted). I hesitate to call it "rationality", because rationality may have more expansive requirements, or may apply also to other realms of human activity. I will therefore call it rationality* in this paper[^2]. I will assume that credences themselves inherit the normative status of the actions which they will make an agent choose: Say, according to the agent's credences and utilities, the expected utility of action $A$ is $-1$, and the expected utility of action $\neg A$ is $1$; but action $\neg A$ is recognised to be irrational*. Then the agent's credences are irrational*. I will consider the paradigmatic example of such an action to be: Entering into a set of bets which the agent recognises to jointly guarantee a loss of utility—It can never be rational* to give away utility for nothing when keeping it is also permitted; Consequently, a set of credences is in any case irrational* if it makes one susceptible to a Durch Book.
3. (Less importantly, but relevant for at least the application of the framework discussed in section @sec-application): I assume that the language game of giving and asking for reasons includes some norm like "If you asserted $p$ yesterday, and had good reasons for it, and have not become aware of any new reasons bearing on the truth of $p$; then you are still committed to $p$."

I will start (like MacFarlane does in his paper) by showcasing the utility of the concept of "credence" in describing and explaining human behaviour. I will then move on to describe the two distinctive talents which the concept of "belief" has above and beyond what "credence" can do, and introducing the specific job which MacFarlane has in mind for it in within our theorising of the human being and it's actions. The result will be a form of belief-credence dualism, whose explanatory power, as well as perhaps counter-intuitive consequences will be showcased in applying it to the "Sleeping Beauty" problem in section @sec-application.

Section @sec-pineal will raise the question of whether perhaps this dualism has pulled belief and credence so far apart as to make unintelligible how they interact. It will answer this question by turning to the practice of betting as a bridge between the two attitudes.

Finally, the section @sec-prediction will apply the framework to the initial question with which this paper started---what is the normative pull of a bet's market price on our beliefs and credences.

# A MacFarlane-Inspired Belief-Credence Dualism

## The Power of Credence

Belief-first theorist Kenneth @easwaran2016DrTruthlove presents as the main talent of the concept of "credence" its ability to show how a high certainty in each of a large set of claims doesn't necessarily entail a high certainty in their conjunction---thus offering an elegant solution to the preface paradox[^3]. 

But viewing the preface paradox as the only, or even primary, arena in which credence excels, is selling it short. Much more impressive is the work it can do to rationalise and explain a large class of human actions as part of decision theory. In this domain, it fares far better than "belief" does.

Here is an example from @macfarlane2025BeliefWhat :
...
First, belief has *truth* as its correctness condition. If $p$ turns out not to be true, then belief that $p$ was wrong. Whereas even arbitrarily high credence in $p$ may not be proven wrong by $p$ being shown not to obtain---even credence 1 does not, strictly speaking, rule out $\neg p$. As @williamson2020knowledge (as cited by MacFarlane) points out with the example of a (countably) infinite series of coin tosses: We certainly should have credence $1$ in the proposition that at least one of them will come up tails; and this credence will not have shown to have been wrong if---which is possible---not a single toss does come up tails. (The set of possible world in which this happens has measure zero; but it is not empty.)

Second, belief---unlike credence---can serve as the basis for certain reactive attitudes, as described by @buchakBeliefCredenceNorms2014 : I cannot resent you for stealing my phone unless I believe you did. Credence, even an arbitrarily high one, is not enough.
...
To the extent that the result of the communication is the formation, in the individual, of new beliefs, these beliefs should never feature in an explanation of that individual's actions---that is credence's job! And belief should not interfere with it, since it's talents do not qualify it or this job.
...
Successful execution of a bet between to people from start to finish involves and connects both decision theory and reasoning, both credence and belief (I take general latent awareness of this fact to be the reason why Dutch Book arguments are often used to convince novices of Bayesianism, and, as I will expand on later, are in fact the only correct justification of it). An individual's action within such a process is guided, at times, both by decision theory and by the normative requirements to which one is subject as a reasoner (and thus, believer). The way these are connected through a (real or imagined) bet will impose indirect epistemic requirements on credence, even if the only direct normative requirements on credence are pragmatic.
...
\[...spell out the relationship between (hypothetical) bets with people and "bets with nature"; this will work via sth like "consider any 'bet with nature' as expressing your credences (which, in the framework we are developing, it in fact does); and therefore as implying that you would enter into certain bets"...\]

# Prediction Markets {#sec-prediction}
